...
.DESCRIPTION
    ...
    Export-IntuneRemediations.ps1 and Push-RemdiationsToIntune.ps1 scripts, then
    launches the chosen script with the matching parameters. No Intune logic is
    duplicated here - this is purely a front-end launcher.
...
.PARAMETER Path
    ...
    own folder. Passed through to the Export/Push scripts as needed.

.EXAMPLE
    .\Start-RemediationToolkit.ps1
...
# Resolve the sibling script paths so the launcher works from any directory
$script:ExportScript = Join-Path $PSScriptRoot 'Export-IntuneRemediations.ps1'
$script:PushScript   = Join-Path $PSScriptRoot 'Push-RemdiationsToIntune.ps1'
...
# Sanity check that the worker scripts are present before showing the menu
foreach ($s in @($script:ExportScript, $script:PushScript)) {
    if (-not (Test-Path $s)) {
...
switch ($StartAt) {
    'Push'   { Invoke-PushMenu;   exit 0 }
    'Export' { Invoke-ExportMenu; exit 0 }
}
